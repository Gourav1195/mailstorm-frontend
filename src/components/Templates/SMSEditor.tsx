...
const TemplateEditor: React.FC<TemplateEditorProps> = ({
  ...
  const [form, setForm] = useState<Template>({
    ...
    content: '',
    testPhoneNumber: template?.testPhoneNumber || '',
  });
  ...
  const handleContentChange : any = (value: string) => {
  ...
  return (
    <Box sx={{
      ...
      <Box component="form" onSubmit={handleSubmit} >
        ...
        >
          ...
          <Box>
            ...
            {/* <Button variant="outlined" onClick={()=> setOpenTest(true)} sx={{ minWidth: '160px', p: 1, m: 1, bgcolor: 'white', color: '#0057D9', '&:hover': { bgcolor: '#e6e6e6', }, }}>
              <SendIcon />&nbsp; Send&nbsp;Test
            </Button> */}
            ...
            <Button /*type="submit"*/ onClick={handleTestModal} variant="contained" color="success" sx={{ minWidth: '180px', p: 1, m: 1, ":hover": { bgcolor: 'green' } }}>
      ...
      >
        ...
        >
          <Typography sx={{ color: 'white', overflow:'hidden' }}>{form.name}</Typography>
          <IconButton onClick={() => setOpen(false)}>
            <CloseIcon sx={{ color: 'white' }} />
          </IconButton>
        </Box>

        <Box sx={{ minHeight: '350px', p: 2, overflowX:'hidden', overflowY:'auto' }}>
        {/* Subject: {form.subject}<br /><br /> */}
          {form.content}<br />
      ...
      >
        ...
        >
          <Typography sx={{ color: 'white' }}>Send Test Message</Typography>
          <IconButton onClick={() => setOpenTest(false)}>
            <CloseIcon sx={{ color: 'white' }} />
          </IconButton>
        </Box>

        <Box sx={{ minHeight: '150px', p: 2 }}>
          <FormControl variant="outlined" size="small" sx={{ minWidth: {xs: '100%'}, bgcolor: "#F8F9FA", borderRadius: "6px", marginRight:"auto" }}>
            ...
            onChange={(e) => setForm((prev) => ({ ...prev, testPhoneNumber: e.target.value }))}
            name="+1 (555) 123-4567"
            sx={{
              fontSize: "14px",
              width: "100%",
        ...
        <DialogActions>
          <Button variant="outlined" onClick={()=>setOpenTest(false)} sx={{color:'#6D6976', bgcolor: '#EBEBEB', border:'none'}}>Cancel</Button>
          <Button
