...
export default function SamplesDrawer({ templateDetails, setTemplateDetails, error }: SamplesDrawerProps) {
  ...
  const insertBlock = (type: BlockType) => {
    ...
    const defaults: Record<BlockType, any> = {
      ...
      Text:    { text: 'Lorem ipsum…', align: 'left' },
      Button:  { text: 'Button', url: '#' },
      Image:   { url: 'https://assets.usewaypoint.com/sample-image.jpg', alt: '' },
      Html:    { html: '<p>HTML snippet</p>' },
      Avatar:  { src: 'https://placehold.co/100',   alt: 'Avatar' },
      Spacer:  { size: 20 },
      Divider: {},
  ...
  return (
    ...
    >
      <Stack spacing={1} py={1} px={2} width={SAMPLES_DRAWER_WIDTH - 17} justifyContent="space-between" height="100%" mt={2} >
      {error && <Alert severity="error">{error}</Alert>}
          ...
          <FormControl fullWidth variant="outlined" size="medium" sx={{ minWidth: '92%',  bgcolor: "#F8F9FA", borderRadius: "6px", 
          ...
          <Typography variant="h6">Blocks</Typography>
          <Box sx={{overflowX:'auto', height:'400px', display:'flex', flexDirection:'column'}}>
          {BLOCK_TYPES.map((type) => (
            <Button
              key={type}
              // variant="outlined"
              ...
              startIcon={BLOCK_TYPE_ICONS[type]}
              sx={{
                color: "#232232", fontSize:'16px', p:1, // border:'2px solid #ECEEF6', 
