...
const AudienceSelector: React.FC<AudienceSelectorProps> = ({ mode, handleChange, campaignData, audienceName, setAudienceName }) => {
  ...
  useEffect(() => {
    if (campaignData.audience && filters.length > 0) {
      ...
      if (selectedAudience) {
        ...
        dispatch(setAudienceName1(selectedAudience.name));
      }
    }
  }, [campaignData.audience, filters]);
  ...
  return (
    <Box sx={{ boxSizing: 'border-box',bgcolor: ds.colors.surfaceElevated,color: ds.colors.textPrimary }}>
      <Box display="flex"
        ...
        <Button variant="contained" onClick={() => {navigate("/create-filters?isRedirect=true"); dispatch(setSelectedCampaignData(campaignData));}}
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        />
        {audienceName &&(
        <Card sx={{  bgcolor: '#FAF9F9',p: 1, borderRadius:1, mb:0.7, ml:1 }}>    
              ...
              <Grid size={{ xs: 12 }}>
                <Card
                  ...
                  onClick={() => {
                    ...
                    setAudienceName1(audience.name);
                  }}
                >
                  <CardActionArea>
                    <CardContent sx={{ display: 'flex' }}>
                      <Box
                        component="img"
                        src={`${process.env.PUBLIC_URL}/icons/criteriaBased_Campaign.png`}
