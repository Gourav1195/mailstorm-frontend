...
const Step2Templates: React.FC<Step2TemplatesProps> = ({ handleChange, campaignData, templateData, setTemplateData }) => {
  ...
  const handleRedirect = () =>{
    dispatch(setSelectedCampaignData(campaignData));
    if(currentTab === 'Email'){
      navigate('/build-template?isRedirect=true');
    }
    else{
      navigate('/build-sms?isRedirect=true');
    }
  }
    ...
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', m:3 }}>
      <Box>
        ...
        <FormControl variant="outlined" size="small" sx={{ minWidth: { xs: 225 }, bgcolor: "#F8F9FA", borderRadius: "6px", marginRight: "auto", mt: 1 }}>
          ...
          {templateData.name && 
      <Card sx={{ p: 1, bgcolor: '#FAF9F9', borderRadius: 1, ml:1, mb:0.5 }}>
        <Typography sx={{ color: '#2068D5' }}>Selected Template</Typography>
        <Typography variant="body1" fontWeight="bold" sx={{ m: 0.5, }}>{templateData.name}</Typography>
      </Card>
      }
      </Box>
            ...
            {filteredTemplates.map((template) => (
              <Card key={template._id?.toString()}
                ...
                {template?.type === 'Email' ? (
                  ...
                  <Box sx={{
                    position: 'absolute', maxWidth: { xs: 160, sm: 180 }, overflow: 'hidden',
                ...
                <CardContent>
                  ...
                  <Typography sx={{ alignItems: "baseline", color: "#495057", fontSize: { xs: '12px', sm: '14px' }, }}>
                    <IconButton onClick={(e) => {
                      e.stopPropagation();
                      handleViewTemplate(template._id, template.type as "Email" | "SMS");
                ...
                {template && template._id === openIndex && (
                  <SMSPreview   
                      ...
                      includeOptOut={template?.includeOptOutText}
                    />
                  )}
              </Card>
            ))}
